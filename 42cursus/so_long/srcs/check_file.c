...
/*   Updated: 2022/02/24 14:50:54 by hannkim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_wall(char *line, int row, int *col)
{
	while (*line == '1')
	{
		line++;
...
void	parsing(char *file, t_list *map)
{
	...
	{
		if (row == 0)
			check_wall(line, row, &col);
		else
			check_line(line, col, flag);
	...
	if (*line)
		...
		check_wall(ft_lstlast(map)->content, row, &col);
}
